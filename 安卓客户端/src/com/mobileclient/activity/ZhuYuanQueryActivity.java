...
public class ZhuYuanQueryActivity extends Activity {
	// 声明查询按钮
	...
	/*查询过滤条件保存到这个对象中*/
	private ZhuYuan queryConditionZhuYuan = new ZhuYuan();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.zhuyuan_query);
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("设置住院查询条件");
		...
		spinner_patientObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionZhuYuan.setPatientObj(patientList.get(arg2-1).getPatiendId()); 
				else
					queryConditionZhuYuan.setPatientObj(0);
			}
			@Override
		...
		spinner_doctorObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionZhuYuan.setDoctorObj(doctorList.get(arg2-1).getDoctorNo()); 
				else
					queryConditionZhuYuan.setDoctorObj("");
			}
			@Override
			public void onNothingSelected(AdapterView<?> arg0) {}
		});
			...
			public void onClick(View v) {
				try {
					...
					if(cb_inDate.isChecked()) {
						...
						queryConditionZhuYuan.setInDate(new Timestamp(inDate.getTime()));
					} else {
						queryConditionZhuYuan.setInDate(null);
					} 
					queryConditionZhuYuan.setBedNum(ET_bedNum.getText().toString());
					Intent intent = getIntent();
					//这里使用bundle绷带来传输数据
					Bundle bundle =new Bundle();
					//传输的内容仍然是键值对的形式
					bundle.putSerializable("queryConditionZhuYuan", queryConditionZhuYuan);
