...
public class DrugUseAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.druguse_add); 
			...
			public void onClick(View v) {
				try {
					...
					if(ET_drugCount.getText().toString().equals("")) {
						Toast.makeText(DrugUseAddActivity.this, "用药数量输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_drugMoney.getText().toString().equals("")) {
						Toast.makeText(DrugUseAddActivity.this, "药品费用输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_useTime.getText().toString().equals("")) {
						Toast.makeText(DrugUseAddActivity.this, "用药时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传用药信息*/
					DrugUseAddActivity.this.setTitle("正在上传用药信息，稍等...");
					String result = drugUseService.AddDrugUse(drugUse);
