...
public class DoctorAction extends BaseAction {
    ...
    public String AddDoctor() {
        ...
        if(null != db_doctor) {
            ctx.put("error",  java.net.URLEncoder.encode("该医生工号已经存在!"));
            return "error";
        }
        try {
            Department departmentObj = departmentDAO.GetDepartmentByDepartmentNo(doctor.getDepartmentObj().getDepartmentNo());
    ...
    public String QueryDoctor() {
        ...
        List<Doctor> doctorList = doctorDAO.QueryDoctorInfo(doctorNo, departmentObj, name, cardNo, birthday, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        doctorDAO.CalculateTotalPageAndRecordNumber(doctorNo, departmentObj, name, cardNo, birthday, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryDoctorOutputToExcel() { 
        ...
        if(birthday == null) birthday = "";
        if(telephone == null) telephone = "";
        List<Doctor> doctorList = doctorDAO.QueryDoctorInfo(doctorNo,departmentObj,name,cardNo,birthday,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Doctor信息记录"; 
        String[] headers = { "医生工号","所在科室","姓名","性别","身份证号","医生照片","出生日期","联系电话","毕业学校","工作经验"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<doctorList.size();i++) {
        	Doctor doctor = doctorList.get(i); 
        	dataset.add(new String[]{doctor.getDoctorNo(),doctor.getDepartmentObj().getDepartmentName(),
doctor.getName(),doctor.getSex(),doctor.getCardNo(),doctor.getDoctorPhoto(),new SimpleDateFormat("yyyy-MM-dd").format(doctor.getBirthday()),doctor.getTelephone(),doctor.getSchool(),doctor.getWorkYears()});
        }
        /*
